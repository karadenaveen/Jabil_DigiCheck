  ...
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS \`templates\` (
        ...
        \`grid_data\` LONGTEXT DEFAULT NULL,
    ...
    // 13b. Self-healing column migration for databases created before
    // grid_data/original_file_path/grid_answers/filled_excel_path existed.
    // `CREATE TABLE IF NOT EXISTS` above is a no-op on a table that already
    // exists, so an older database never picks up newer columns just by
    // restarting — this explicitly adds any that are missing. Safe to run
    // on every startup: MySQL error 1060 (ER_DUP_FIELDNAME) means the
    // column is already there, which is expected and ignored.
    const ensureColumn = async (table, columnName, definition) => {
      try {
        await pool.query(`ALTER TABLE \`${table}\` ADD COLUMN \`${columnName}\` ${definition};`);
        logger.info(`Migration: added missing column ${table}.${columnName}`);
      } catch (error) {
        if (error.code !== 'ER_DUP_FIELDNAME') {
          logger.warn(`Migration: could not add ${table}.${columnName}:`, error.message);
        }
      }
    };

    await ensureColumn('templates', 'grid_data', 'LONGTEXT DEFAULT NULL AFTER `cover_page`');
    await ensureColumn('templates', 'original_file_path', 'VARCHAR(255) DEFAULT NULL AFTER `grid_data`');
