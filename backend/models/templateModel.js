...
export const templateModel = {
  getTemplates: async ({ page = 1, limit = 50, search = '', status = 'All' }) => {
    ...
    const sql = `
      SELECT id, title, short_title as shortTitle, doc_number as docNumber, revision, 
             ...
             status, sheets_count as sheetsCount, cover_page as coverPage, grid_data as gridData, original_file_path as originalFilePath, created_at
      FROM templates
      ${baseSql}
    ...
    const templatesWithSheets = await Promise.all(rows.map(async (tmpl) => {
      ...
      let gridData = null;
      try {
        gridData = tmpl.gridData ? JSON.parse(tmpl.gridData) : null;
      } catch {
        gridData = null;
      }
      ...
      return {
        ...
        coverPage: cover,
        gridData,
        // No fabricated fallback row — an empty sheets array is a real,
        // honest empty state (template has no mapped checklist rows yet).
        sheets
      };
    }));
  ...
  createTemplate: async (templateData, dbConnection = null) => {
    ...
    const gridDataJson = templateData.gridData ? JSON.stringify(templateData.gridData) : null;

    const sql = `
      INSERT INTO templates (id, title, short_title, doc_number, revision, category, uploaded_by, uploaded_date, status, sheets_count, cover_page, grid_data, created_by, updated_by)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
    `;
  ...
  setOriginalFilePath: async (id, filePath) => {
    await pool.query(`
      UPDATE templates SET original_file_path = ? WHERE id = ?;
    `, [filePath, id]);
    return templateModel.getTemplateById(id);
  },

  updateGridData: async (id, gridData) => {
    await pool.query(`
      UPDATE templates SET grid_data = ? WHERE id = ?;
    `, [JSON.stringify(gridData), id]);
    return templateModel.getTemplateById(id);
  }
};
